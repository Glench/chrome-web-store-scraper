...
class ChromeWebStoreScraper {
    ...
    async scrapeAppHeader(driver) {
        ...
        html = await body[0].getAttribute('innerHTML')
        header.userCount = parseInt(html.match(/user_count">(\d+)/)[1], 10)
        return header
    }
    ...
    parseAppHeaderHTML(html) {
        ...
        const rating = parseFloat($('.Y89Uic').attr('title').replace('Average rating ', '').split(' ')[0]) || null;
        const userCount = parseInt($('.e-f-ih').first().text().replace(/\D/g, ''));
        const ratingCount = parseInt($('.nAtiRe').text().replace(/\D/g, ''));
        const buy_button_text = $('#cxdialog-buy-text').text();
        var price = null;
        var price_type = null;
        if (buy_button_text) {
            var price = buy_button_text.match(/\d+(.\d+)?/)
            if (price) {
                price = parseFloat(price[0])
            }
            var price_type = 'once'
            if (buy_button_text.includes('year')) {
                price_type = 'year'
            } else if (buy_button_text.includes('month')) {
                price_type = 'month'
            }
        }
        ...
        return {
            title : title,
            price: price_type == 'once' ? price : null,
            price_per_month: price_type == 'month' ? price : null,
            price_per_year: price_type == 'year' ? price : null,
            offeredBy : offeredBy,
            userCount : userCount,
            rating : rating,
